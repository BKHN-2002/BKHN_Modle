...
function InstructorFiller()
{
    ...
    $courseId = $_GET['courseId'];
    $sql = "SELECT `id`, `name` FROM `instructor` WHERE id IN (SELECT `instructorID` FROM `instructor_course` WHERE  `courseId` =  $courseId )  ";
    $result = mysqli_query($GLOBALS['conn'], $sql);
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $instructorId = $row['id'];
            ...
            <div class="content-header">
                <form action="" method="post">
                    ...
                    <div class="instructorId-container">
                        <label class="label" for="">Instructor ID : </label>
                        <div class="ct-select-group ct-js-select-group">
